    ...
    const handleLogin = async () => {
      try {
        ...
        if (usuario) {
          ...
          const firstAccess = await dbService.getFirstAccessById(id);
          const name = await dbService.getNameById(id);

          setUser({
            id: id,
            name: name ? name : usuario.nome,
            email: email,
            primeiro_acesso: 0
          });

          if(firstAccess === 1){
            const _ = await dbService.updateFirstLogin(id);
